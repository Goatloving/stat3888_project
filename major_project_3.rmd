...
dat33.balanced <-smote(DIABBC~., dat33, perc.over=600, perc.under=100)
...
model<-nnet::multinom(dia_score~., data=train.data)  # multiclass logsitic regression model
...
# Kaggle body fat dataset

```{r kaggle dataset}
kdat<-read.csv("bodyfat.csv")
ggpairs(kdat)

skim(kdat)
```

```{r kaggle lasso regression}

x_var <- model.matrix(BodyFat~., kdat)[,-2]
y_var<-kdat$BodyFat
...
rsq
```
```{r kaggle ridge regression}
set.seed(2021)
kdat_for_ahs<-kdat[,c(1:5,8)]

x_var <- model.matrix(BodyFat~., kdat_for_ahs)[,-2]
x_var
x_var
...
lasso_best<-glmnet(x_train, y_train, alpha = 0, lambda=best_lam)
...
Assume we use the above equation in AHS data
The equation would be BodyFat = -50.83-0.006(Age)-0.104(Weight, lbs)+0.032(Height)+0.939(Waist/Abdomen)

```{r apply kaggle model in AHS}
BFD<- -0.006*dat$AGEC-0.104*dat$PHDKGWBC/0.45359237+0.032*dat$PHDCMHBC +0.939*dat$PHDCMWBC # need to convert kg to lbs

kdat2 <-cbind(dat3, BFD)
kdat2
